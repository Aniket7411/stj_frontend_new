...
function FindCandidate() {
  ...
  const fetchCandidates = async () => {
    try {
      const response = await HttpClient.get(`/user/find`);
      ...
      const formattedData = data.map((candidate) => ({
        ...
        title:
          ...
          "Unknown",
        dob: candidate?.profile?.personalInformation?.dob,
        nationality: candidate?.profile?.personalInformation?.nationality,
        skills: candidate?.profile?.personalInformation?.skills,

        paypalEmail: candidate?.profile?.personalInformation?.paypalEmail,
        ailments: candidate?.profile?.generalInformation?.ailments,
        bio: candidate?.profile?.generalInformation?.bio,
        paypalEmail: candidate?.profile?.personalInformation?.paypalEmail,
        experience: candidate?.profile?.generalInformation?.experience,
        firstAid: candidate?.profile?.generalInformation?.firstAid,
        ukDrivingLicense: candidate?.profile?.generalInformation?.ukDrivingLicense,
        healthIssue: candidate?.profile?.generalInformation?.healthIssue,
        utrNumber: candidate?.profile?.generalInformation?.utrNumber,
        workPermit: candidate?.profile?.generalInformation?.workPermit,
        millitaryBackground: candidate?.profile?.generalInformation?.millitaryBackground,
        paramedicTraining: candidate?.profile?.generalInformation?.paramedicTraining,
        bankDetails: candidate?.profile?.uploads?.bankDetails,
        drivingLicense: candidate?.profile?.uploads?.drivingLicense,
        educationalCertificate: candidate?.profile?.uploads?.educationalCertificate,
        skillCertificate: candidate?.profile?.uploads?.skillCertificate,
        userId: candidate?.userId,

      }));
      console.log("datadatadata", formattedData)
  ...
  const makeFavourite = async (candidateId) => {
    ...
    try {
      ...
      await HttpClient.post("favCandidate/favorite", { candidateId });
          ...
          <div className="p-4">
            <div className="flex flex-wrap justify-between space-x-2 shadow-lg lg:rounded-full sm:rounded-lg mt-3 p-3 items-center z-10">
              <Link to="/findcandidate">
                <button className=" border-lg px-3 py-1 text-[#c5363c] outline h-auto w-auto ">
                  Find Candidate
                </button>
              </Link>
                ...
                {filteredCandidates.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm text-gray-600">
                            {candidate.candidateProfession || "N/A"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="text-right">
                          <button
                            onClick={() =>
                              navigate(`/userprofile/${candidate?.id}`, {
                                state: { candidate },
                              })
                            }
                          >
                  ...
                  <div className="p-6 flex justify-center items-center w-auto text-gray-500 bg-gray-100 rounded-lg">
                    <p className="text-center">
                      No candidates found. Please try changing the Job Title
                      filter.
                    </p>
                  </div>
                )}
